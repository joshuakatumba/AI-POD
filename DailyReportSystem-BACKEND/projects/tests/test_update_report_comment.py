...
class TestUpdateReportComment(MockAuthMixin, APITestCase):
    def setUp(self):
    ...
    def test_patch_comment_outsider_org_returns_not_found(self):
    ...
    def test_patch_comment_non_author_org_member_forbidden(self):
        self.client.force_authenticate(self.other_member_user)
        with self.mock_auth(self.other_member_auth):
            response = self.client.patch(self.url, {"content": "Unauthorized"}, format="json")
